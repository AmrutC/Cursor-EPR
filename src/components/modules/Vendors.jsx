...
export default function Vendors() {
  ...
  return (
    <div>
      {/* Tabs */}
      <div style={{ display:'flex', gap:3, background:'var(--bg-subtle)', borderRadius:10, padding:3, marginBottom:14, width:'fit-content' }}>
        {[['vendors','Vendor Master'],['bills','Bill Register'],['pending','Pending Payment'],['po','Purchase Orders']].map(([id,label])=>(
          <button key={id} onClick={()=>setTab(id)}
            style={{ padding:'6px 18px', borderRadius:7, fontSize:12.5, fontWeight:tab===id?700:500, color:tab===id?'var(--c-dark)':'var(--t-secondary)', background:tab===id?'#fff':'transparent', cursor:'pointer', border:tab===id?'1px solid var(--border)':'1px solid transparent', boxShadow:tab===id?'0 1px 2px rgba(0,0,0,0.06)':'' }}>
            {label}
          </button>
        ))}
      </div>
      ...
      {tab==='vendors' && (
        <div>
          ...
          {filtered.map(v=>{
            ...
            return (
              <div key={v.id} style={{ background:'#fff', border:'1px solid var(--border)', borderRadius:14, padding:'16px 18px', marginBottom:10, boxShadow:'0 1px 3px rgba(0,0,0,0.04)' }}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:12 }}>
                  <div>
                    <div style={{ fontSize:15, fontWeight:800, color:'var(--c-dark)', marginBottom:3 }}>{v.name}</div>
                    <div style={{ fontSize:12, color:'var(--t-secondary)' }}>{v.type} · {v.phone} {v.email&&`· ${v.email}`}</div>
                    {v.gstin && <div style={{ fontSize:11.5, color:'var(--t-primary)', fontFamily:'monospace', marginTop:2 }}>GSTIN: {v.gstin}</div>}
                    {v.pan  && <div style={{ fontSize:11.5, color:'var(--t-primary)', fontFamily:'monospace' }}>PAN: {v.pan}</div>}
                  </div>
                  <div style={{ display:'flex', gap:7, alignItems:'center' }}>
                    <Badge value={v.status}/>
                    <button onClick={()=>{ setVForm({...v}); setEditVId(v.id); setVendorModal(true); }} style={{ background:'var(--bg-subtle)', border:'1px solid var(--border)', borderRadius:7, padding:'4px 10px', cursor:'pointer', fontSize:11.5, fontWeight:600, color:'var(--t-primary)', display:'flex', alignItems:'center', gap:4 }}><Edit2 size={10}/> Edit</button>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:8 }}>
                  {[['Contract Value',inr(v.contract_value||0,true),'var(--c-dark)'],['Total Billed',inr(totalBilled,true),'var(--t-secondary)'],['Total Paid',inr(totalPaid,true),'var(--c-success)'],['Balance',inr(totalBilled-totalPaid,true),totalBilled-totalPaid>0?'#7F1D1D':'var(--t-primary)']].map(([l,val,c])=>(
                    <div key={l} style={{ background:'var(--bg-subtle)', borderRadius:8, padding:'8px 11px' }}>
                    ...
                    <table style={{ width:'100%', borderCollapse:'collapse', minWidth:800 }}>
                      <thead><tr style={{ background:'var(--bg-subtle)', borderBottom:'1px solid var(--border)' }}>
                        {['Invoice','Date','Description','Taxable','GST','TDS','Total Bill','Net Payable','Paid','Balance','Status',''].map(h=>(
                          <th key={h} style={{ padding:'6px 10px', textAlign:'left', fontSize:9.5, fontWeight:700, color:'var(--t-secondary)', textTransform:'uppercase', letterSpacing:'0.4px', whiteSpace:'nowrap' }}>{h}</th>
                        ))}
                      </tr></thead>
                      <tbody>
                        {(v.bills||[]).map((b,i)=>(
                          <tr key={b.id} style={{ borderBottom:'1px solid var(--bg-subtle)', background:i%2===0?'#fff':'#FAFAFA' }}>
                            <td style={{ padding:'7px 10px', fontSize:11.5, fontFamily:'monospace', fontWeight:600, color:'var(--c-dark)' }}>{b.invoice_no}</td>
                            <td style={{ padding:'7px 10px', fontSize:11.5, color:'var(--t-primary)' }}>{fmtDate(b.invoice_date)}</td>
                            <td style={{ padding:'7px 10px', fontSize:12, color:'var(--t-primary)', maxWidth:160, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{b.description}</td>
                            <td style={{ padding:'7px 10px', fontSize:12, fontFamily:'monospace', textAlign:'right', color:'var(--t-primary)' }}>{inr(b.taxable_value)}</td>
                            <td style={{ padding:'7px 10px', fontSize:12, fontFamily:'monospace', textAlign:'right', color:b.gst_applicable?'var(--t-secondary)':'var(--t-muted)' }}>{b.gst_applicable?inr(b.gst_total):'—'}</td>
                            <td style={{ padding:'7px 10px', fontSize:12, fontFamily:'monospace', textAlign:'right', color:b.tds_applicable?'var(--c-primary)':'var(--t-muted)' }}>{b.tds_applicable?`-${inr(b.tds_amount)}`:'—'}</td>
                            <td style={{ padding:'7px 10px', fontSize:12.5, fontFamily:'monospace', textAlign:'right', fontWeight:700, color:'var(--c-dark)' }}>{inr(b.total_bill)}</td>
                            <td style={{ padding:'7px 10px', fontSize:12.5, fontFamily:'monospace', textAlign:'right', fontWeight:700, color:'var(--c-success)' }}>{inr(b.net_payable)}</td>
                            <td style={{ padding:'7px 10px', fontSize:12, fontFamily:'monospace', textAlign:'right', color:'var(--c-success)' }}>{inr(b.paid_amount)}</td>
                            <td style={{ padding:'7px 10px', fontSize:12, fontFamily:'monospace', textAlign:'right', color:b.net_payable-b.paid_amount>0?'var(--c-danger)':'var(--c-success)', fontWeight:700 }}>{inr(b.net_payable-b.paid_amount)}</td>
                            ...
                            <td style={{ padding:'7px 10px' }}>
                              {b.status!=='Paid' && (
                                <button onClick={()=>{ setPayModal({...b,vendor_id:v.id}); setPayAmount(String(b.net_payable-b.paid_amount)); setPayDate(new Date().toISOString().slice(0,10)); }}
                                  style={{ background:'var(--c-success-light)', border:'1px solid var(--c-success)', borderRadius:6, padding:'2px 8px', cursor:'pointer', fontSize:10.5, fontWeight:700, color:'var(--c-success)' }}>Pay</button>
                              )}
                            </td>
                          </tr>
                        ))}
        ...
        <div style={{ background:'#fff', border:'1px solid var(--border)', borderRadius:14, overflow:'hidden', boxShadow:'0 1px 3px rgba(0,0,0,0.04)' }}>
          <div style={{ overflowX:'auto' }}>
            <table style={{ width:'100%', borderCollapse:'collapse', minWidth:900 }}>
              ...
              <tbody>
                {allBills.length===0?<tr><td colSpan={10} style={{ textAlign:'center', padding:40, color:'var(--t-muted)', fontSize:13 }}>No bills recorded yet.</td></tr>:
                allBills.map((b,i)=>(
                  <tr key={b.id} style={{ borderBottom:'1px solid var(--bg-subtle)', background:i%2===0?'#fff':'#FAFAFA' }}>
                    <td style={{ padding:'9px 12px', fontSize:12.5, fontWeight:600, color:'var(--c-dark)' }}>{b.vendor_name}</td>
                    <td style={{ padding:'9px 12px', fontSize:11.5, fontFamily:'monospace', fontWeight:700, color:'var(--t-primary)' }}>{b.invoice_no}</td>
                    <td style={{ padding:'9px 12px', fontSize:12.5, color:'var(--t-primary)' }}>{fmtDate(b.invoice_date)}</td>
                    <td style={{ padding:'9px 12px', fontSize:12, color:'var(--t-primary)' }}>{b.description}</td>
                    <td style={{ padding:'9px 12px', fontSize:12, fontFamily:'monospace', textAlign:'right', color:'var(--t-primary)' }}>{inr(b.taxable_value)}</td>
                    <td style={{ padding:'9px 12px', fontSize:12, fontFamily:'monospace', textAlign:'right', color:'var(--t-secondary)' }}>{b.gst_applicable?inr(b.gst_total):'—'}</td>
                    <td style={{ padding:'9px 12px', fontSize:12, fontFamily:'monospace', textAlign:'right', color:'var(--c-primary)' }}>{b.tds_applicable?inr(b.tds_amount):'—'}</td>
                    <td style={{ padding:'9px 12px', fontSize:12.5, fontFamily:'monospace', textAlign:'right', fontWeight:700, color:'var(--c-dark)' }}>{inr(b.total_bill)}</td>
                    <td style={{ padding:'9px 12px', fontSize:12.5, fontFamily:'monospace', textAlign:'right', fontWeight:700, color:'var(--c-success)' }}>{inr(b.net_payable)}</td>
          ...
          <table style={{ width:'100%', borderCollapse:'collapse' }}>
            <thead><tr style={{ background:'var(--bg-subtle)', borderBottom:'2px solid var(--border)' }}>
              {['Vendor','Invoice No.','Net Payable','Paid','Balance',''].map(h=>(
                <th key={h} style={{ padding:'9px 14px', textAlign:'left', fontSize:10, fontWeight:700, color:'var(--t-secondary)', textTransform:'uppercase', letterSpacing:'0.4px', whiteSpace:'nowrap' }}>{h}</th>
              ))}
            </tr></thead>
            <tbody>
              {allBills.filter(b=>b.status!=='Paid').length===0
                ?<tr><td colSpan={6} style={{ textAlign:'center', padding:40, color:'var(--t-muted)', fontSize:13 }}>No pending payments.</td></tr>
                :allBills.filter(b=>b.status!=='Paid').map((b,i)=>(
                  <tr key={b.id} style={{ borderBottom:'1px solid var(--bg-subtle)', background:i%2===0?'#fff':'#FAFAFA' }}>
                    <td style={{ padding:'9px 14px', fontSize:13, fontWeight:700, color:'var(--c-dark)' }}>{b.vendor_name}</td>
                    <td style={{ padding:'9px 14px', fontSize:11.5, fontFamily:'monospace', color:'var(--t-primary)' }}>{b.invoice_no}</td>
                    <td style={{ padding:'9px 14px', fontSize:13, fontFamily:'monospace', fontWeight:700, textAlign:'right', color:'var(--c-dark)' }}>{inr(b.net_payable)}</td>
                    <td style={{ padding:'9px 14px', fontSize:13, fontFamily:'monospace', textAlign:'right', color:'var(--c-success)' }}>{inr(b.paid_amount)}</td>
                    <td style={{ padding:'9px 14px', fontSize:13, fontFamily:'monospace', textAlign:'right', fontWeight:800, color:'var(--c-danger)' }}>{inr(b.net_payable-b.paid_amount)}</td>
                    <td style={{ padding:'9px 14px' }}>
                      <button onClick={()=>{ setPayModal({...b}); setPayAmount(String(b.net_payable-b.paid_amount)); setPayDate(new Date().toISOString().slice(0,10)); }}
                        style={{ background:'var(--c-success-light)', border:'1px solid var(--c-success)', borderRadius:6, padding:'4px 10px', cursor:'pointer', fontSize:11, fontWeight:700, color:'var(--c-success)' }}>
                        Record Payment
                      </button>
                    </td>
                  </tr>
      ...
      <Modal open={billModal} onClose={()=>{ setBillModal(false); setBForm(EMPTY_BILL); }}
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:13 }}>
          ...
          <div style={{ gridColumn:'1/-1' }}>
            <div style={{ display:'flex', alignItems:'center', gap:10, padding:'10px 12px', background:'var(--bg-subtle)', borderRadius:10, marginBottom:bForm.gst_applicable?12:0 }}>
              ...
              <label htmlFor="gst_chk" style={{ fontSize:13, fontWeight:700, color:'var(--t-primary)', cursor:'pointer' }}>GST Applicable?</label>
            </div>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8, gridColumn:'1/-1' }}>
                  {bForm.supply_type==='CGST+SGST'?(<>
                    <div style={{ background:'#fff', borderRadius:8, padding:'8px 11px' }}>
                      <div style={{ fontSize:10, fontWeight:700, color:'var(--t-secondary)', textTransform:'uppercase', marginBottom:3 }}>CGST</div>
                      <div style={{ fontSize:13, fontWeight:800, color:'var(--t-secondary)', fontFamily:'monospace' }}>{inr(bForm.cgst)}</div>
                    </div>
                    <div style={{ background:'#fff', borderRadius:8, padding:'8px 11px' }}>
                      <div style={{ fontSize:10, fontWeight:700, color:'var(--t-secondary)', textTransform:'uppercase', marginBottom:3 }}>SGST</div>
                      <div style={{ fontSize:13, fontWeight:800, color:'var(--t-secondary)', fontFamily:'monospace' }}>{inr(bForm.sgst)}</div>
                    </div>
                  </>):(
                    <div style={{ background:'#fff', borderRadius:8, padding:'8px 11px' }}>
                      <div style={{ fontSize:10, fontWeight:700, color:'var(--t-secondary)', textTransform:'uppercase', marginBottom:3 }}>IGST</div>
                      <div style={{ fontSize:13, fontWeight:800, color:'var(--t-secondary)', fontFamily:'monospace' }}>{inr(bForm.igst)}</div>
                    </div>
                  )}
                  <div style={{ background:'#EAF0F8', borderRadius:8, padding:'8px 11px' }}>
                    <div style={{ fontSize:10, fontWeight:700, color:'var(--c-primary)', textTransform:'uppercase', marginBottom:3 }}>Total GST</div>
                    <div style={{ fontSize:13, fontWeight:800, color:'var(--c-primary)', fontFamily:'monospace' }}>{inr(bForm.gst_total)}</div>
                  </div>
                </div>
              </div>
            )}
          ...
          {/* TDS */}
          <div style={{ gridColumn:'1/-1' }}>
            <div style={{ display:'flex', alignItems:'center', gap:10, padding:'10px 12px', background:'var(--bg-subtle)', borderRadius:10, marginBottom:bForm.tds_applicable?12:0 }}>
              ...
              <label htmlFor="tds_chk" style={{ fontSize:13, fontWeight:700, color:'var(--t-primary)', cursor:'pointer' }}>TDS Applicable?</label>
            </div>
            {bForm.tds_applicable && (
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:13, padding:'12px', background:'#F1E8FF', borderRadius:10, border:'1px solid #D4B9FF' }}>
                ...
                <div style={{ background:'#F1E8FF', borderRadius:8, padding:'10px 12px' }}>
                  <div style={{ fontSize:10, fontWeight:700, color:'var(--c-info)', textTransform:'uppercase', marginBottom:3 }}>TDS Deduction</div>
                  <div style={{ fontSize:16, fontWeight:800, color:'var(--c-info)', fontFamily:'monospace' }}>{inr(bForm.tds_amount)}</div>
                  <div style={{ fontSize:10.5, color:'var(--t-secondary)', marginTop:2 }}>= Taxable Value × {bForm.tds_rate}%</div>
                </div>
              </div>
            )}
          </div>
          ...
          {/* Net Payable summary */}
          <div style={{ gridColumn:'1/-1', background:'var(--c-dark)', borderRadius:10, padding:'12px 16px', display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:10 }}>
            {[['Total Bill',inr(bForm.total_bill),'var(--border)'],['TDS Deducted',inr(bForm.tds_amount),'#D4B9FF'],['Net Payable',inr(bForm.net_payable),'var(--c-success)']].map(([l,v,c])=>(
              <div key={l}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:13 }}>
            <div style={{ gridColumn:'1/-1', background:'#EAF0F8', border:'1px solid #C5D5E8', borderRadius:10, padding:'10px 14px', fontSize:12.5 }}>
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8 }}>
                <div><span style={{ color:'var(--t-secondary)', fontWeight:600 }}>Total Bill: </span><strong style={{ fontFamily:'monospace' }}>{inr(payModal.total_bill)}</strong></div>
                <div><span style={{ color:'var(--t-secondary)', fontWeight:600 }}>TDS Deducted: </span><strong style={{ color:'var(--c-info)', fontFamily:'monospace' }}>{inr(payModal.tds_amount)}</strong></div>
                <div><span style={{ color:'var(--t-secondary)', fontWeight:600 }}>Net Payable: </span><strong style={{ color:'var(--c-success)', fontFamily:'monospace' }}>{inr(payModal.net_payable-payModal.paid_amount)}</strong></div>
              </div>
            </div>
      ...
      {tab==='po' && (
        <div>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14 }}>
            <div style={{ fontSize:13, color:'var(--t-secondary)' }}>Generate PO for any registered vendor. Opens as a Word (.docx) file.</div>
          ...
          <div style={{ background:'#EAF0F8', border:'1px solid #C5D5E8', borderRadius:12, padding:'14px 18px' }}>
            <div style={{ fontSize:12, fontWeight:700, color:'var(--c-primary)', marginBottom:8 }}>How PO Generation Works</div>
            <div style={{ fontSize:12, color:'var(--t-primary)', lineHeight:1.7 }}>
          ...
          <div style={{ position:'fixed', inset:0, zIndex:60, display:'flex', alignItems:'center', justifyContent:'center', padding:16 }}>
            ...
            <div style={{ position:'relative', background:'#fff', borderRadius:18, width:'100%', maxWidth:900, maxHeight:'92vh', display:'flex', flexDirection:'column', boxShadow:'0 24px 64px rgba(0,0,0,0.22)' }}>
              <div style={{ padding:'16px 22px', borderBottom:'1px solid var(--border)', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                <div style={{ fontSize:15, fontWeight:800, color:'var(--c-dark)' }}>New Purchase Order</div>
                <button onClick={()=>setPoModal(false)} style={{ background:'var(--bg-subtle)', border:'none', borderRadius:8, width:28, height:28, cursor:'pointer', color:'var(--t-secondary)', fontSize:16 }}>✕</button>
              </div>
              ...
              <div style={{ flex:1, overflowY:'auto', padding:'18px 22px' }}>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:13, marginBottom:16 }}>
                  <div>
                    <label style={{ display:'block', fontSize:10, fontWeight:700, color:'var(--t-secondary)', textTransform:'uppercase', letterSpacing:'0.5px', marginBottom:5 }}>Vendor <span style={{ color:'var(--c-danger)' }}>*</span></label>
                    ...
                    {selVend && (
                      <div style={{ marginTop:6, background:'var(--c-success-light)', border:'1px solid var(--c-success)', borderRadius:8, padding:'8px 10px', fontSize:11.5 }}>
                        <div style={{ fontWeight:700, color:'var(--c-dark)' }}>{selVend.name}</div>
                        {selVend.phone && <div style={{ color:'var(--t-primary)' }}>📞 {selVend.phone}</div>}
                        {selVend.gstin && <div style={{ fontFamily:'monospace', color:'var(--t-primary)' }}>GSTIN: {selVend.gstin}</div>}
                      </div>
                    )}
                  </div>
                  <div>
                    <label style={{ display:'block', fontSize:10, fontWeight:700, color:'var(--t-secondary)', textTransform:'uppercase', letterSpacing:'0.5px', marginBottom:5 }}>Project / Delivery Site</label>
                    ...
                    {selProj && (
                      <div style={{ marginTop:6, background:'#EAF0F8', border:'1px solid #C5D5E8', borderRadius:8, padding:'8px 10px', fontSize:11.5 }}>
                        <div style={{ fontWeight:700, color:'var(--c-dark)' }}>Delivery to: {selProj.name}</div>
                        <div style={{ color:'var(--t-primary)' }}>{selProj.village}, {selProj.taluka}, {selProj.district} — {selProj.pin}</div>
                      </div>
                    )}
                  </div>
                  <div style={{ display:'grid', gridTemplateColumns:'1fr', gap:12 }}>
                    <div>
                      <label style={{ display:'block', fontSize:10, fontWeight:700, color:'var(--t-secondary)', textTransform:'uppercase', letterSpacing:'0.5px', marginBottom:5 }}>PO Date</label>
                      <input style={inp} type="date" value={poDate} onChange={e=>setPoDate(e.target.value)}/>
                    </div>
                    <div>
                      <label style={{ display:'block', fontSize:10, fontWeight:700, color:'var(--t-secondary)', textTransform:'uppercase', letterSpacing:'0.5px', marginBottom:5 }}>GST Rate (%)</label>
                ...
                {/* Items table */}
                <div style={{ background:'var(--bg-subtle)', border:'1px solid var(--border)', borderRadius:12, overflow:'hidden', marginBottom:16 }}>
                  <div style={{ background:'var(--c-dark)', padding:'9px 14px', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                    ...
                    <button onClick={addPoItem} style={{ background:'var(--c-warning)', border:'none', borderRadius:6, padding:'4px 12px', cursor:'pointer', fontSize:11, fontWeight:700, color:'var(--c-dark)', display:'flex', alignItems:'center', gap:4 }}>
                    ...
                    <table style={{ width:'100%', borderCollapse:'collapse', minWidth:700 }}>
                      <thead><tr style={{ background:'var(--bg-subtle)', borderBottom:'2px solid var(--border)' }}>
                        {['#','Description of Items / Work','Unit','Qty','Rate (₹)','Amount (₹)',''].map(h=>(
                          <th key={h} style={{ padding:'8px 10px', textAlign:'left', fontSize:10, fontWeight:700, color:'var(--t-secondary)', textTransform:'uppercase', letterSpacing:'0.4px', whiteSpace:'nowrap' }}>{h}</th>
                        ))}
                      </tr></thead>
                      <tbody>
                        {poItems.map((item,i)=>(
                          <tr key={item.id} style={{ borderBottom:'1px solid var(--bg-subtle)' }}>
                            <td style={{ padding:'6px 10px', fontSize:12, color:'var(--t-secondary)', width:30 }}>{i+1}</td>
                            ...
                            <td style={{ padding:'6px 10px', fontSize:13, fontWeight:700, fontFamily:'monospace', textAlign:'right', color:'var(--c-dark)' }}>
                              {inr(Number(item.qty||0)*Number(item.rate||0))}
                            </td>
                            <td style={{ padding:'6px 8px' }}>
                              {poItems.length>1 && (
                                <button onClick={()=>removePoItem(item.id)} style={{ background:'var(--c-danger-light)', border:'1px solid var(--c-danger)', borderRadius:6, padding:'3px 7px', cursor:'pointer', color:'#7F1D1D', fontSize:11 }}>✕</button>
                              )}
                            </td>
                          </tr>
                        ))}
                  ...
                  {/* Totals */}
                  <div style={{ background:'var(--c-dark)', padding:'12px 16px', display:'flex', justifyContent:'flex-end', gap:24 }}>
                    {[['Sub Total', inr(subtotal),'var(--border)'],['GST @'+poGstRate+'%', inr(gstAmt),'var(--c-warning-light)'],['Grand Total', inr(grandTotal),'var(--c-warning)']].map(([l,v,c])=>(
                ...
                <div style={{ background:'var(--bg-subtle)', border:'1px solid var(--border)', borderRadius:12, padding:'14px 16px', marginBottom:14 }}>
                  <div style={{ fontSize:10, fontWeight:700, color:'var(--c-dark)', textTransform:'uppercase', letterSpacing:'0.5px', marginBottom:12 }}>Requisition & Shipping Details</div>
                  <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))', gap:12 }}>
                    <div>
                      <label style={{ display:'block', fontSize:10, fontWeight:700, color:'var(--t-secondary)', textTransform:'uppercase', letterSpacing:'0.5px', marginBottom:5 }}>Requesting Department</label>
                    ...
                    <div>
                      <label style={{ display:'block', fontSize:10, fontWeight:700, color:'var(--t-secondary)', textTransform:'uppercase', letterSpacing:'0.5px', marginBottom:5 }}>Requested By (Name)</label>
                      <input style={inp} value={poReqBy} onChange={e=>setPoReqBy(e.target.value)} placeholder="Full name of requester"/>
                    </div>
                    <div>
                      <label style={{ display:'block', fontSize:10, fontWeight:700, color:'var(--t-secondary)', textTransform:'uppercase', letterSpacing:'0.5px', marginBottom:5 }}>Shipping Method</label>
                    ...
                    <div>
                      <label style={{ display:'block', fontSize:10, fontWeight:700, color:'var(--t-secondary)', textTransform:'uppercase', letterSpacing:'0.5px', marginBottom:5 }}>Site Contact Person</label>
                      <input style={inp} value={poContactName} onChange={e=>setPoContactName(e.target.value)} placeholder="Name at delivery site"/>
                    </div>
                    <div>
                      <label style={{ display:'block', fontSize:10, fontWeight:700, color:'var(--t-secondary)', textTransform:'uppercase', letterSpacing:'0.5px', marginBottom:5 }}>Contact Mobile</label>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:13, marginBottom:4 }}>
                  <div>
                    <label style={{ display:'block', fontSize:10, fontWeight:700, color:'var(--t-secondary)', textTransform:'uppercase', letterSpacing:'0.5px', marginBottom:5 }}>Notes / Special Instructions</label>
                  ...
                  <div>
                    <label style={{ display:'block', fontSize:10, fontWeight:700, color:'var(--t-secondary)', textTransform:'uppercase', letterSpacing:'0.5px', marginBottom:5 }}>Terms &amp; Conditions</label>
              ...
              <div style={{ padding:'14px 22px', borderTop:'1px solid var(--border)', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                <button onClick={()=>setPoModal(false)} style={{ background:'var(--bg-subtle)', border:'1px solid var(--border)', borderRadius:8, padding:'7px 18px', cursor:'pointer', fontSize:13, fontWeight:600, color:'var(--t-primary)' }}>Cancel</button>
                <button onClick={generatePO} disabled={poGenerating} style={{ background:'var(--c-dark)', color:'#fff', border:'none', borderRadius:8, padding:'8px 22px', cursor:'pointer', fontSize:13, fontWeight:700, display:'flex', alignItems:'center', gap:8, opacity:poGenerating?0.6:1 }}>
                  <FileText size={14}/>
