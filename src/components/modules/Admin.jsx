    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '60vh' }}>
        <div style={{ textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 17, fontWeight: 800, color: '#071437' }}>Admin Access Only</div>
          <div style={{ fontSize: 13, color: '#4B5675', marginTop: 6 }}>Contact your Super Admin for access.</div>
        </div>
      </div>
    );
  }
...
// ── SHARED ─────────────────────────────────────────────────────────────────
const Btn = ({ children, onClick, color = '#F8285A', small, disabled, style = {} }) => (
...
const Badge = ({ label, color = '#F8285A' }) => <span style={{ background: color + '18', color, fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: 10 }}>{label}</span>;
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        <div style={{ fontSize: 15, fontWeight: 800, color: '#071437' }}>Entities & Users</div>
      ...
      {subTab === 'entities' && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 14 }}>
          {entities.map(e => (
            <div key={e.id} style={{ background: '#fff', borderRadius: 12, border: '1px solid #F1F1F4', padding: 18 }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 10 }}>
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ background: '#071437', color: '#F6C000', fontSize: 9, fontWeight: 800, padding: '2px 7px', borderRadius: 4 }}>{e.code}</span>
                    <span style={{ fontSize: 14, fontWeight: 800, color: '#071437' }}>{e.name}</span>
                  </div>
                  <div style={{ fontSize: 10, color: '#78829D', marginTop: 3 }}>{e.cin_llpin || 'CIN/LLPIN not set'}</div>
                </div>
                <Badge label={e.type || 'pvt_ltd'} color="#7239EA" />
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 10, fontSize: 11 }}>
                {[['GSTIN', e.gstin || '—'], ['PAN', e.pan || '—'], ['Phone', e.phone || '—'], ['Signatory', e.authorized_signatory || '—']].map(([k, v]) => (
                  <div key={k}><span style={{ color: '#78829D' }}>{k}: </span><span style={{ color: '#252F4A', fontWeight: 500 }}>{v}</span></div>
                ))}
              </div>
              {e.address && <div style={{ fontSize: 10, color: '#78829D', marginBottom: 10 }}>{e.address}</div>}
              <button onClick={() => { setEditEntity(e); setEntityForm({ ...e }); setModal('entity'); }} style={{ fontSize: 11, color: '#F8285A', background: 'none', border: '1px solid #F8285A', borderRadius: 6, padding: '4px 10px', cursor: 'pointer' }}>Edit</button>
            </div>
          ))}
        </div>
      )}
            ...
            <tbody>
              {users.map(u => (
                <tr key={u.id}>
                  ...
                  <Td><Badge label={ROLE_LABELS[u.role] || u.role} color="#F8285A" /></Td>
                  <Td style={{ fontSize: 10 }}>{(u.entity_access || []).map(id => entities.find(e => e.id === id)?.code).filter(Boolean).join(', ') || 'None'}</Td>
                  <Td><Badge label={u.is_active ? 'Active' : 'Inactive'} color={u.is_active ? '#17C653' : '#4B5675'} /></Td>
                  <Td>
                    <button onClick={() => { setEditUser(u); setUserForm({ ...u }); setModal('user'); }} style={{ background: '#FFE2E5', color: '#F8285A', border: 'none', borderRadius: 6, padding: '4px 8px', fontSize: 11, cursor: 'pointer' }}>Edit</button>
                  </Td>
                </tr>
              ))}
            </tbody>
      ...
      {modal === 'entity' && (
        <ModalOverlay onClose={() => setModal(null)} title={editEntity ? 'Edit Entity' : 'Add Entity'} wide>
          ...
          <div style={{ marginTop: 12 }}>
            <label style={{ fontSize: 11, fontWeight: 600, color: '#252F4A', display: 'block', marginBottom: 4 }}>Address</label>
            <textarea value={entityForm.address || ''} onChange={e => setEntityForm(f => ({ ...f, address: e.target.value }))} rows={2} style={{ width: '100%', padding: '7px 10px', border: '1px solid #F1F1F4', borderRadius: 8, fontSize: 12, resize: 'vertical', boxSizing: 'border-box' }} />
          </div>
      ...
      {modal === 'user' && (
        <ModalOverlay onClose={() => setModal(null)} title={editUser ? 'Edit User' : 'Add User'} wide>
          ...
          <div style={{ marginBottom: 14 }}>
            <label style={{ fontSize: 11, fontWeight: 600, color: '#252F4A', display: 'block', marginBottom: 8 }}>Entity Access</label>
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              {entities.map(e => (
                <label key={e.id} style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer', background: (userForm.entity_access || []).includes(e.id) ? '#FFE2E5' : '#FCFCFC', borderRadius: 8, padding: '6px 12px', fontSize: 12, fontWeight: 600, border: '1px solid ' + ((userForm.entity_access || []).includes(e.id) ? '#FFB8C6' : '#F1F1F4') }}>
...
const LOCK_LEVELS = [
  { value: 'active', label: '🟢 Active', color: '#17C653' },
  { value: 'view_only', label: '🟡 View Only', color: '#F6C000' },
  { value: 'locked', label: '🔴 Locked', color: '#F8285A' },
];
...
function ModuleLocksTab() {
  ...
  return (
    <div>
      <div style={{ fontSize: 15, fontWeight: 800, color: '#071437', marginBottom: 4 }}>Module Locks</div>
      <div style={{ fontSize: 11, color: '#78829D', marginBottom: 16 }}>Control per-entity module access. Super Admin & Director bypass all locks.</div>

      <div style={{ display: 'flex', gap: 10, marginBottom: 20, alignItems: 'center' }}>
        <select value={selEntity} onChange={e => setSelEntity(e.target.value)} style={{ border: '1px solid #F1F1F4', borderRadius: 8, padding: '7px 12px', fontSize: 12, minWidth: 220 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 24 }}>
        {MODULE_LIST.map(mod => {
          ...
          return (
            <div key={mod.id} style={{ background: '#fff', borderRadius: 12, border: '1px solid #F1F1F4', padding: 16 }}>
              <div style={{ fontWeight: 700, fontSize: 13, color: '#071437', marginBottom: 12 }}>{mod.label}</div>
              <div style={{ display: 'flex', gap: 8 }}>
                {LOCK_LEVELS.map(ll => (
      ...
      {/* Priority Matrix */}
      <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #F1F1F4', overflow: 'hidden' }}>
        <div style={{ padding: '12px 16px', borderBottom: '1px solid #FCFCFC', fontWeight: 700, fontSize: 13, color: '#071437' }}>Role Priority Matrix</div>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead><tr><Th>Role</Th><Th>Lock Behavior</Th></tr></thead>
          <tbody>
            {PRIORITY_MATRIX.map(r => (
              <tr key={r.role}>
                <Td style={{ fontWeight: 600 }}><Badge label={ROLE_LABELS[r.role] || r.role} color="#F8285A" /></Td>
                <Td style={{ color: r.role === 'super_admin' || r.role === 'director' ? '#17C653' : '#252F4A', fontWeight: r.role === 'super_admin' ? 700 : 400 }}>{r.note}</Td>
              </tr>
            ))}
          </tbody>
        </table>
...
function MISTab() {
  ...
  return (
    <div>
      <div style={{ fontSize: 15, fontWeight: 800, color: '#071437', marginBottom: 16 }}>MIS & Reports — {activeEntity?.name}</div>

      {/* KPI Summary */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14, marginBottom: 24 }}>
        {[
          ['Total Bookings', stats.totalBookings, '#1B84FF'],
          ['Approved Bookings', stats.approvedBookings, '#17C653'],
          ['Total Booking Value', `₹${(stats.totalBookingValue / 10000000).toFixed(2)} Cr`, '#7239EA'],
          ['Total Collected', `₹${(stats.totalCollected / 100000).toFixed(1)}L`, '#0E9F8A'],
          ['TDS Deducted', `₹${stats.totalTDS.toLocaleString('en-IN')}`, '#F8285A'],
          ['GST Outward', `₹${stats.totalGSTOut.toLocaleString('en-IN')}`, '#F6C000'],
        ].map(([l, v, c]) => (
      ...
      {/* Export Reports */}
      <div style={{ fontWeight: 700, fontSize: 13, color: '#071437', marginBottom: 12 }}>Export Reports</div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 10 }}>
        {REPORTS.map(r => (
          <div key={r.label} style={{ background: '#fff', borderRadius: 10, border: '1px solid #F1F1F4', padding: '14px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div>
              <div style={{ fontWeight: 600, fontSize: 13, color: '#071437' }}>{r.label}</div>
              <div style={{ fontSize: 10.5, color: '#78829D', marginTop: 2 }}>{r.desc} · {r.data?.length || 0} records</div>
            </div>
            <Btn onClick={() => exportCSV(r.data || [], r.file)} small color="#1B84FF"><Download size={12} /> CSV</Btn>
          </div>
        ))}
      </div>
    </div>
...
function AuditLogTab() {
  ...
  const ACTION_COLORS = { ADD: '#17C653', EDIT: '#1B84FF', DELETE: '#F8285A', APPROVE: '#0E9F8A', CANCEL: '#F8285A', LOGIN: '#7239EA' };

  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        <div style={{ fontSize: 15, fontWeight: 800, color: '#071437' }}>Audit Log</div>
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <select value={filterModule} onChange={e => setFilterModule(e.target.value)} style={{ border: '1px solid #F1F1F4', borderRadius: 8, padding: '6px 10px', fontSize: 12 }}>
      ...
      <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #F1F1F4', overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {filtered.slice(0, 200).map(e => {
              ...
              return (
                <tr key={e.id}>
                  <Td style={{ fontSize: 10.5, color: '#78829D', whiteSpace: 'nowrap' }}>{new Date(e.ts).toLocaleString('en-IN')}</Td>
                  <Td style={{ fontWeight: 500 }}>{e.user || '—'}</Td>
                  <Td><Badge label={e.action || '—'} color={ACTION_COLORS[actionKey] || '#4B5675'} /></Td>
                  <Td><Badge label={e.module || '—'} color="#1B84FF" /></Td>
        ...
        {filtered.length === 0 && <div style={{ padding: 32, textAlign: 'center', color: '#78829D', fontSize: 13 }}>No audit entries yet. Actions you perform will appear here.</div>}
      </div>
    </div>
  );
}
...
function BackupManagerTab() {
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        <div>
          <div style={{ fontSize: 15, fontWeight: 800, color: '#071437' }}>Backup Manager</div>
          <div style={{ fontSize: 11, color: '#78829D' }}>Auto-backup runs daily on startup. Manual backup anytime.</div>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
      ...
      {/* Sync folder info */}
      {oneDrivePath && (
        <div style={{ background: '#E8FFF3', borderRadius: 10, padding: '10px 16px', marginBottom: 16, fontSize: 12, color: '#17C653' }}>
      ...
      <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #F1F1F4', overflow: 'hidden' }}>
        {backups.length === 0 ? (
          <div style={{ padding: 48, textAlign: 'center', color: '#78829D', fontSize: 13 }}>
            <Database size={36} style={{ color: '#F1F1F4', marginBottom: 12 }} />
...
function TallyImportTab() {
  ...
  return (
    <div>
      <div style={{ fontSize: 15, fontWeight: 800, color: '#071437', marginBottom: 4 }}>Tally Import</div>
      <div style={{ fontSize: 11, color: '#78829D', marginBottom: 20 }}>Import data from Tally XML export. Review before committing.</div>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: 20 }}>
        {/* Left: Controls */}
        <div>
          <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #F1F1F4', padding: 16, marginBottom: 14 }}>
            <div style={{ fontWeight: 700, fontSize: 13, color: '#071437', marginBottom: 12 }}>Import Settings</div>
            <div style={{ marginBottom: 12 }}>
              <label style={{ fontSize: 11, fontWeight: 600, color: '#252F4A', display: 'block', marginBottom: 6 }}>Import Type</label>
              {TALLY_IMPORT_TYPES.map(t => (
                <label key={t.id} style={{ display: 'flex', alignItems: 'flex-start', gap: 8, marginBottom: 8, cursor: 'pointer' }}>
                  ...
                  <div>
                    <div style={{ fontSize: 12, fontWeight: 600, color: '#071437' }}>{t.label}</div>
                    <div style={{ fontSize: 10, color: '#78829D' }}>{t.desc}</div>
                  </div>
                </label>
              ))}
            </div>
            <div style={{ marginBottom: 14 }}>
              <label style={{ fontSize: 11, fontWeight: 600, color: '#252F4A', display: 'block', marginBottom: 6 }}>Conflict Resolution</label>
        ...
        {/* Right: Preview */}
        <div>
          <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #F1F1F4', padding: 16 }}>
            <div style={{ fontWeight: 700, fontSize: 13, color: '#071437', marginBottom: 12 }}>Preview</div>
            {!preview ? (
              <div style={{ padding: '32px 0', textAlign: 'center', color: '#78829D', fontSize: 13 }}>Load an XML file to preview data before importing.</div>
            ) : (
              <>
                <div style={{ background: '#E8FFF3', borderRadius: 8, padding: '10px 14px', marginBottom: 14 }}>
                  <div style={{ fontWeight: 700, fontSize: 13, color: '#17C653' }}>{preview.count} records found</div>
                  <div style={{ fontSize: 11, color: '#17C653', marginTop: 2 }}>File: {preview.fileName}</div>
                </div>
                <div style={{ background: '#FCFCFC', borderRadius: 8, padding: '10px 14px', marginBottom: 14, fontFamily: 'monospace', fontSize: 10, color: '#252F4A', maxHeight: 200, overflowY: 'auto' }}>
                  {preview.raw}…
                </div>
                <div style={{ background: '#FFF8DD', borderRadius: 8, padding: '10px 14px', marginBottom: 14, fontSize: 11.5, color: '#7A4E00' }}>
                  ⚠ Review carefully. Import cannot be automatically undone — take a backup first.
                </div>
                <div style={{ display: 'flex', gap: 8 }}>
                  <Btn onClick={() => setPreview(null)} color="#4B5675" small>Cancel</Btn>
                  <Btn onClick={importData} color="#17C653" small>Confirm Import ({preview.count} records, {resolution})</Btn>
                </div>
              </>
            )}
          </div>
...
function NotificationCentreTab() {
  ...
  const TYPE_COLORS = { info: '#1B84FF', warning: '#F6C000', error: '#F8285A', success: '#17C653' };

  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        <div>
          <div style={{ fontSize: 15, fontWeight: 800, color: '#071437' }}>Notification Centre</div>
          <div style={{ fontSize: 11, color: '#78829D' }}>{unread} unread · {notifications.length} total</div>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          {unread > 0 && <Btn onClick={markAllNotifRead} color="#4B5675" small>Mark All Read</Btn>}
      ...
      <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #F1F1F4', overflow: 'hidden' }}>
        {notifications.length === 0 ? (
          <div style={{ padding: 48, textAlign: 'center', color: '#78829D', fontSize: 13 }}>
            <Bell size={36} style={{ color: '#F1F1F4', marginBottom: 12 }} />
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {notifications.slice(0, 100).map(n => (
                <tr key={n.id} style={{ background: n.read ? '#fff' : '#EEF6FF' }}>
                  <Td style={{ fontSize: 10.5, color: '#78829D', whiteSpace: 'nowrap' }}>{new Date(n.ts).toLocaleString('en-IN')}</Td>
                  <Td style={{ fontWeight: n.read ? 400 : 600 }}>{n.message}</Td>
                  <Td><Badge label={n.type || 'info'} color={TYPE_COLORS[n.type] || '#1B84FF'} /></Td>
                  <Td>{n.module || '—'}</Td>
                  <Td>{n.read ? <Badge label="Read" color="#4B5675" /> : <Badge label="Unread" color="#1B84FF" />}</Td>
                  <Td>
                    {!n.read && <button onClick={() => markNotifRead(n.id)} style={{ background: 'none', border: 'none', color: '#1B84FF', cursor: 'pointer', fontSize: 11 }}>Mark Read</button>}
                  </Td>
                </tr>
              ))}
            </tbody>
      ...
      {modal && (
        <ModalOverlay onClose={() => setModal(false)} title="Send Notification">
          ...
          <div style={{ marginBottom: 16 }}>
            <label style={{ fontSize: 11, fontWeight: 600, color: '#252F4A', display: 'block', marginBottom: 4 }}>Message</label>
            <textarea value={form.message} onChange={e => setForm(f => ({ ...f, message: e.target.value }))} rows={3} style={{ width: '100%', padding: '7px 10px', border: '1px solid #F1F1F4', borderRadius: 8, fontSize: 12, resize: 'vertical', boxSizing: 'border-box' }} />
          </div>
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
