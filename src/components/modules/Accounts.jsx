        ...
        {filtered.length===0
          ...
          :<table style={{ width:'100%', borderCollapse:'collapse' }}>
            <thead><tr style={{ background:'#F9FAFB', borderBottom:'2px solid #E5E7EB' }}>
              ...
              <th style={{ padding:'9px 12px', fontSize:10, fontWeight:700, color:'#4B5563', textTransform:'uppercase', letterSpacing:'0.4px', whiteSpace:'nowrap', position:'sticky', right:0, background:'#F9FAFB', boxShadow:'-2px 0 4px rgba(0,0,0,0.06)' }}>Actions</th>
            </tr></thead>
            <tbody>
              {filtered.map((e,i)=>{
                    ...
                    <td style={{ padding:'7px 10px', whiteSpace:'nowrap', position:'sticky', right:0, background:rowBg, boxShadow:'-2px 0 4px rgba(0,0,0,0.06)' }}>
                      <div style={{ display:'flex', gap:4 }}>
                        <button onClick={()=>setViewEntry(e)}
                          title="View Details"
                          style={{ background:'#EAF0F8', border:'none', borderRadius:6, padding:'4px 7px', cursor:'pointer', color:'#0D1E35', display:'flex', alignItems:'center' }}>
                          <Eye size={11}/>
                        </button>
                        <button onClick={()=>openEdit(e)}
                          title="Edit Entry"
                          style={{ background:'#FEF3C7', border:'none', borderRadius:6, padding:'4px 7px', cursor:'pointer', color:'#78350F', display:'flex', alignItems:'center' }}>
                          <Edit2 size={11}/>
                        </button>
                        <button onClick={()=>deleteEntry(e.id)}
                          title="Delete Entry"
                          style={{ background:'#FEE2E2', border:'none', borderRadius:6, padding:'4px 7px', cursor:'pointer', color:'#7F1D1D', display:'flex', alignItems:'center' }}>
                          <Trash2 size={11}/>
                        </button>
                      </div>
                    </td>
        ...
        <div style={{ position:'fixed', inset:0, zIndex:60, display:'flex', alignItems:'center', justifyContent:'center', padding:16 }}>
          ...
          <div style={{ position:'relative', background:'#fff', borderRadius:16, width:'100%', maxWidth:580, maxHeight:'88vh', display:'flex', flexDirection:'column', boxShadow:'0 24px 64px rgba(0,0,0,0.2)' }}>
            <div style={{ padding:'13px 20px', borderBottom:'1px solid #E5E7EB', display:'flex', justifyContent:'space-between', alignItems:'center', flexShrink:0 }}>
              <div>
                <div style={{ fontSize:14, fontWeight:800, color:'#0D1E35' }}>Ledger Entry Details</div>
                <div style={{ fontSize:11, color:'#9CA3AF', marginTop:2 }}>{fmtDate(viewEntry.date)} · {viewEntry.project_name}</div>
              </div>
              <div style={{ display:'flex', gap:6 }}>
                <button onClick={()=>{ setViewEntry(null); openEdit(viewEntry); }}
                  style={{ background:'#FEF3C7', border:'1px solid #FCD34D', borderRadius:7, padding:'5px 12px', cursor:'pointer', fontSize:12, fontWeight:700, color:'#78350F', display:'flex', alignItems:'center', gap:5 }}>
                  <Edit2 size={11}/> Edit
                </button>
                <button onClick={()=>setViewEntry(null)}
                  style={{ background:'#F3F4F6', border:'none', borderRadius:7, width:26, height:26, cursor:'pointer', color:'#6B7280', display:'flex', alignItems:'center', justifyContent:'center' }}>✕</button>
              </div>
            </div>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:0 }}>
                {[
                  ...
                  <div key={label} style={{ padding:'8px 0', borderBottom:'1px solid #F3F4F6' }}>
                    <div style={{ fontSize:9.5, fontWeight:700, color:'#6B7280', textTransform:'uppercase', letterSpacing:'0.4px', marginBottom:2 }}>{label}</div>
                    <div style={{ fontSize:13, fontWeight:600, color:'#0D1E35' }}>{value}</div>
                  </div>
                ))}
              </div>
            </div>
            <div style={{ padding:'12px 20px', borderTop:'1px solid #E5E7EB', display:'flex', gap:8, justifyContent:'space-between', flexShrink:0 }}>
              <button onClick={()=>{ if(confirm('Delete this entry?')){ deleteEntry(viewEntry.id); setViewEntry(null); } }}
                style={{ background:'#FEE2E2', border:'1px solid #FCA5A5', borderRadius:8, padding:'7px 14px', cursor:'pointer', fontSize:12.5, fontWeight:700, color:'#7F1D1D', display:'flex', alignItems:'center', gap:5 }}>
                <Trash2 size={12}/> Delete
              </button>
              <button onClick={()=>setViewEntry(null)}
                style={{ background:'#F3F4F6', border:'1px solid #E5E7EB', borderRadius:8, padding:'7px 18px', cursor:'pointer', fontSize:13, fontWeight:600, color:'#374151' }}>Close</button>
            </div>
          </div>
        </div>
      )}
