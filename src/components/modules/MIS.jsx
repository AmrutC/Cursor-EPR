...
const FLAT_STATUS = [
  ...
  { name:'Agreement Done',   value:2,  color:'#7A4E00' },
  ...
  return (
    <div>
      ...
      <div style={{display:'flex',gap:6,flexWrap:'wrap',marginBottom:16}}>
        ...
        <button style={{marginLeft:'auto',background:'#FCFCFC',border:'1px solid #F1F1F4',borderRadius:9,padding:'7px 14px',cursor:'pointer',fontSize:12,fontWeight:600,color:'#252F4A',display:'flex',alignItems:'center',gap:6}}>
          ...
          <div style={{background:'#fff',border:'1px solid #F1F1F4',borderRadius:14,padding:'18px 20px',boxShadow:'0 1px 3px rgba(0,0,0,0.04)'}}>
            ...
            <ResponsiveContainer width="100%" height={240}>
              <BarChart data={MONTHS} barCategoryGap="30%" barSize={16}>
                ...
                <Tooltip formatter={v=>[inr(v),'']} labelStyle={{fontSize:12,fontWeight:700,color:'#071437'}} contentStyle={{borderRadius:10,border:'1px solid #F1F1F4',fontSize:12,color:'#252F4A'}}/>
            ...
            <table style={{width:'100%',borderCollapse:'collapse'}}>
              ...
              <tbody>
                {MONTHS.map((m,i)=>{
                  ...
                  return(
                    <tr key={m.month} style={{borderBottom:'1px solid #FCFCFC',background:i%2===0?'#fff':'#FAFAFA'}}>
                      ...
                      <td style={{padding:'9px 16px',fontSize:13,fontFamily:'monospace',textAlign:'right',fontWeight:700,color:(m.demand-m.received)>0?'#7F1D1D':'#17C653'}}>{inr(m.demand-m.received)}</td>
                      <td style={{padding:'9px 16px'}}>
                        <div style={{display:'flex',alignItems:'center',gap:8}}>
                          <div style={{width:80,height:6,background:'#FCFCFC',borderRadius:3,overflow:'hidden'}}>
                            <div style={{width:`${pct}%`,height:'100%',background:pct>=100?'#17C653':pct>=70?'#7A4E00':'#F8285A',borderRadius:3}}/>
                          </div>
                          <span style={{fontSize:12,fontWeight:700,color:pct>=100?'#17C653':pct>=70?'#7A4E00':'#7F1D1D'}}>{pct}%</span>
                        </div>
                      </td>
                    </tr>
                  );
      ...
      {active==='flatstatus'&&(
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14}}>
          ...
          <div style={{background:'#fff',border:'1px solid #F1F1F4',borderRadius:14,overflow:'hidden',boxShadow:'0 1px 3px rgba(0,0,0,0.04)'}}>
            <div style={{padding:'13px 18px',borderBottom:'1px solid #FCFCFC',fontSize:12,fontWeight:700,color:'#071437',textTransform:'uppercase',letterSpacing:'0.5px'}}>Status Summary</div>
            <table style={{width:'100%',borderCollapse:'collapse'}}>
              <thead><tr style={{background:'#FCFCFC',borderBottom:'2px solid #F1F1F4'}}>
              ...
              <tbody>
                {FLAT_STATUS.map((f,i)=>(
                  <tr key={f.name} style={{borderBottom:'1px solid #FCFCFC',background:i%2===0?'#fff':'#FAFAFA'}}>
      ...
      {active==='pl'&&(
        <div style={{display:'flex',flexDirection:'column',gap:14}}>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:10}}>
            {[
              {l:'Total Income',v:inr(totalIncome),c:'#17C653',bg:'#E8FFF3',border:'#50CD89'},
              {l:'Total Expenses',v:inr(totalExpense),c:'#7F1D1D',bg:'#FFE2E5',border:'#FFB8C6'},
              {l:'Net Profit/Loss',v:inr(netPL),c:netPL>=0?'#17C653':'#7F1D1D',bg:netPL>=0?'#E8FFF3':'#FFE2E5',border:netPL>=0?'#50CD89':'#FFB8C6'},
            ].map(s=>(
          ...
          <div style={{background:'#fff',border:'1px solid #F1F1F4',borderRadius:14,overflow:'hidden',boxShadow:'0 1px 3px rgba(0,0,0,0.04)'}}>
            ...
            <table style={{width:'100%',borderCollapse:'collapse'}}>
              ...
              <tbody>
                {PL_DATA.map((r,i)=>(
                  <tr key={r.head} style={{borderBottom:'1px solid #FCFCFC',background:i%2===0?'#fff':'#FAFAFA'}}>
                    ...
                    <td style={{padding:'10px 18px',fontSize:13,fontFamily:'monospace',textAlign:'right',fontWeight:700,color:r.type==='income'?'#17C653':'#7F1D1D'}}>
                ...
                <tr style={{background:'#071437'}}>
                  ...
                  <td style={{padding:'11px 18px',fontSize:14,fontFamily:'monospace',textAlign:'right',fontWeight:800,color:netPL>=0?'#50CD89':'#FFB8C6'}}>{netPL>=0?'+':''}{inr(netPL)}</td>
                </tr>
              </tbody>
            </table>
          </div>
      ...
      {active==='vendor'&&(
        <div style={{background:'#fff',border:'1px solid #F1F1F4',borderRadius:14,overflow:'hidden',boxShadow:'0 1px 3px rgba(0,0,0,0.04)'}}>
          <div style={{padding:'13px 18px',borderBottom:'1px solid #FCFCFC',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
          ...
          <table style={{width:'100%',borderCollapse:'collapse'}}>
            ...
            <tbody>
              {VENDOR_DUES.map((v,i)=>{
                ...
                return(
                  <tr key={v.name} style={{borderBottom:'1px solid #FCFCFC',background:i%2===0?'#fff':'#FAFAFA'}}>
                    ...
                    <td style={{padding:'10px 16px',fontSize:13,fontFamily:'monospace',textAlign:'right',fontWeight:700,color:due>0?'#7F1D1D':'#17C653'}}>{due?inr(due):'Nil'}</td>
                    <td style={{padding:'10px 16px'}}>
                      <div style={{display:'flex',alignItems:'center',gap:6}}>
                        <div style={{width:60,height:5,background:'#FCFCFC',borderRadius:3}}><div style={{width:`${paidPct}%`,height:'100%',background:'#17C653',borderRadius:3}}/></div>
      ...
      {active==='gst'&&(
        <div style={{background:'#fff',border:'1px solid #F1F1F4',borderRadius:14,overflow:'hidden',boxShadow:'0 1px 3px rgba(0,0,0,0.04)'}}>
          <div style={{padding:'13px 18px',borderBottom:'1px solid #FCFCFC',fontSize:12,fontWeight:700,color:'#071437',textTransform:'uppercase',letterSpacing:'0.5px'}}>GST Summary — FY 2026-27</div>
          <table style={{width:'100%',borderCollapse:'collapse'}}>
            <thead><tr style={{background:'#FCFCFC',borderBottom:'2px solid #F1F1F4'}}>
            ...
            <tbody>
              {GST_MO.map((m,i)=>(
                <tr key={m.month} style={{borderBottom:'1px solid #FCFCFC',background:i%2===0?'#fff':'#FAFAFA'}}>
                  ...
                  <td style={{padding:'10px 18px',fontSize:13,fontFamily:'monospace',textAlign:'right',fontWeight:800,color:'#7A4E00'}}>{inr(m.gst)}</td>
                  <td style={{padding:'10px 18px'}}><Badge value="Pending"/></td>
                  <td style={{padding:'10px 18px',fontSize:12,color:'#4B5675'}}>11th of next month</td>
                </tr>
              ))}
              <tr style={{background:'#071437'}}>
      ...
      {active==='cashflow'&&(
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14}}>
          {[
            ...
            {l:'Total Payments',v:inr(399000),c:'#7F1D1D'},{l:'Closing Bank Balance',v:inr(2967524),c:'#071437'},
