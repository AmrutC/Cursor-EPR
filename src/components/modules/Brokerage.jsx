    ...
    <div>
      ...
      <div style={{ display:'flex', gap:3, background:'var(--bg-subtle)', borderRadius:10, padding:3, marginBottom:14, width:'fit-content' }}>
        {[['master','Broker Master'],['payouts','Payout Register']].map(([id,label])=>(
          <button key={id} onClick={()=>setTab(id)}
            style={{ padding:'6px 18px', borderRadius:7, fontSize:12.5, fontWeight:tab===id?700:500, color:tab===id?'var(--c-dark)':'var(--t-secondary)', background:tab===id?'#fff':'transparent', cursor:'pointer', border:tab===id?'1px solid var(--border)':'1px solid transparent' }}>
            {label}
          </button>
        ))}
      </div>
        ...
        <div>
          ...
          {filtered.length===0 ? (
            <div style={{ background:'#fff', border:'1px solid var(--border)', borderRadius:14, padding:60, textAlign:'center' }}>
              ...
              <div style={{ fontSize:14, fontWeight:600, color:'var(--t-secondary)' }}>No brokers registered yet</div>
              <div style={{ fontSize:12, color:'var(--t-muted)', marginTop:4 }}>Registered brokers will appear in the Booking form Step 4</div>
            </div>
          ) : filtered.map(b=>{
            const payouts = getBrokerPayouts(b);
            return (
              <div key={b.id} style={{ background:'#fff', border:'1px solid var(--border)', borderRadius:14, padding:'16px 18px', marginBottom:10, boxShadow:'0 1px 3px rgba(0,0,0,0.04)', cursor:'pointer' }}
                onClick={()=>{ setSelBroker(b); setTab('payouts'); }}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:12 }}>
                  <div>
                    <div style={{ fontSize:15, fontWeight:800, color:'var(--c-dark)', marginBottom:3 }}>{b.firm_name}</div>
                    <div style={{ fontSize:12.5, color:'var(--t-primary)', marginBottom:2 }}>{b.contact_person}</div>
                    <div style={{ display:'flex', gap:12, fontSize:12, color:'var(--t-secondary)', flexWrap:'wrap' }}>
                  ...
                  <div style={{ display:'flex', gap:8, alignItems:'center' }}>
                    ...
                    <button onClick={e=>{ e.stopPropagation(); openEdit(b); }} style={{ background:'var(--bg-subtle)', border:'1px solid var(--border)', borderRadius:7, padding:'4px 10px', cursor:'pointer', fontSize:11.5, fontWeight:600, color:'var(--t-primary)', display:'flex', alignItems:'center', gap:4 }}><Edit2 size={10}/> Edit</button>
                  </div>
                </div>
                <div style={{ display:'grid', gridTemplateColumns:'repeat(5,1fr)', gap:8 }}>
                  {[['PAN',b.pan||'—'],['RERA No.',b.rera_no||'—'],['TDS Rate',`${b.tds_rate||5}% (u/s 194H)`],['Bookings',String(payouts.length)],['Total Brokerage (Gross)',inr(payouts.reduce((s,p)=>s+p.gross_brokerage,0),true)]].map(([l,v])=>(
                    <div key={l} style={{ background:'var(--bg-subtle)', borderRadius:8, padding:'8px 10px' }}>
                      ...
                      <div style={{ fontSize:12.5, fontWeight:700, color:'var(--c-dark)', fontFamily:l==='PAN'||l==='RERA No.'?'monospace':'inherit' }}>{v}</div>
                    </div>
                  ))}
                </div>
                {b.address && <div style={{ marginTop:8, fontSize:12, color:'var(--t-secondary)' }}>{b.address}</div>}
              </div>
            );
          })}
        </div>
        ...
        <div>
          ...
          {selBroker && (<>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:10, marginBottom:14 }}>
              {[['Gross Brokerage',inr(totalGross,true),'var(--t-secondary)','var(--c-warning-light)','var(--c-warning)'],['TDS (u/s 194H)',inr(totalTDS,true),'var(--c-info)','#F1E8FF','#D4B9FF'],['Net Payable',inr(totalNet,true),'var(--c-success)','var(--c-success-light)','var(--c-success)'],['Total Bookings',String(selPayouts.length),'var(--c-dark)','#EAF0F8','#C5D5E8']].map(([l,v,tc,bg,bdr])=>(
            ...
            {selPayouts.length===0 ? (
              <div style={{ background:'#fff', border:'1px solid var(--border)', borderRadius:14, padding:40, textAlign:'center', color:'var(--t-muted)', fontSize:13 }}>No approved bookings found for {selBroker.firm_name}.</div>
            ) : (
                ...
                <table style={{ width:'100%', borderCollapse:'collapse' }}>
                  ...
                  <tbody>
                    {selPayouts.map((p,i)=>(
                      <tr key={i} style={{ borderBottom:'1px solid var(--bg-subtle)', background:i%2===0?'#fff':'#FAFAFA' }}>
                        <td style={{ padding:'9px 12px', fontSize:11.5, fontFamily:'monospace', fontWeight:700, color:'var(--c-dark)' }}>{p.booking_no}</td>
                        <td style={{ padding:'9px 12px', fontSize:12.5, fontWeight:600, color:'var(--c-primary)' }}>Unit {p.unit_no}</td>
                        <td style={{ padding:'9px 12px', fontSize:12.5, color:'var(--t-primary)' }}>{p.allottee}</td>
                        <td style={{ padding:'9px 12px', fontSize:12, fontFamily:'monospace', textAlign:'right', color:'var(--t-primary)' }}>{inr(p.agreement_value)}</td>
                        <td style={{ padding:'9px 12px', fontSize:12.5, fontFamily:'monospace', textAlign:'right', fontWeight:700, color:'var(--t-secondary)' }}>{inr(p.gross_brokerage)}</td>
                        <td style={{ padding:'9px 12px', fontSize:12, fontFamily:'monospace', textAlign:'right', color:'var(--c-info)' }}>-{inr(p.tds)}</td>
                        <td style={{ padding:'9px 12px', fontSize:13, fontFamily:'monospace', textAlign:'right', fontWeight:800, color:'var(--c-success)' }}>{inr(p.net_brokerage)}</td>
                  ...
                  <tfoot>
                    <tr style={{ background:'var(--c-dark)' }}>
                      ...
                      <td style={{ padding:'9px 12px', fontSize:13, fontFamily:'monospace', textAlign:'right', fontWeight:800, color:'var(--c-warning-light)' }}>{inr(totalGross)}</td>
                      <td style={{ padding:'9px 12px', fontSize:13, fontFamily:'monospace', textAlign:'right', fontWeight:800, color:'#D4B9FF' }}>{inr(totalTDS)}</td>
                      <td style={{ padding:'9px 12px', fontSize:13, fontFamily:'monospace', textAlign:'right', fontWeight:800, color:'var(--c-success)' }}>{inr(totalNet)}</td>
                      <td/>
                    </tr>
                  </tfoot>
                </table>
                <div style={{ padding:'11px 14px', background:'var(--c-warning-light)', borderTop:'1px solid var(--c-warning)', fontSize:12, color:'var(--t-secondary)' }}>
      ...
      <Modal open={modal} onClose={()=>{ setModal(false); setErrors({}); }}
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:13 }}>
          ...
          <div style={{ gridColumn:'1/-1', background:'#EAF0F8', border:'1px solid #C5D5E8', borderRadius:9, padding:'9px 13px', fontSize:12, color:'var(--c-primary)' }}>
