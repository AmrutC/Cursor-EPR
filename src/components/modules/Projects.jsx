    ...
    <div>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16 }}>
        ...
        {canEdit
          ...
          : <div style={{ display:'flex', alignItems:'center', gap:6, fontSize:12, color:'#78829D', background:'#F9F9F9', border:'1px solid #F1F1F4', borderRadius:8, padding:'6px 12px' }}>
      ...
      {entityProjects.length===0 ? (
        <div style={{ background:'#fff', border:'1px solid #F1F1F4', borderRadius:14, padding:60, textAlign:'center' }}>
          ...
          <div style={{ fontSize:14, fontWeight:600, color:'#78829D' }}>No projects registered yet</div>
          {canEdit && <div style={{ fontSize:12, color:'#99A1B7', marginTop:4 }}>Click "Register Project" to add your first project</div>}
        </div>
        ...
        return (
          <div key={p.id} style={{ background:'#fff', border:'1px solid #F1F1F4', borderRadius:14, padding:20, marginBottom:12, boxShadow:'0 1px 3px rgba(0,0,0,0.05)' }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:14 }}>
              <div>
                <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:4 }}>
                  ...
                  {p.is_jv && <span style={{ background:'#F1E8FF', color:'#5014D0', fontSize:10.5, fontWeight:700, padding:'2px 8px', borderRadius:10 }}>Joint Venture</span>}
                </div>
              ...
              <div style={{ display:'flex', gap:8, alignItems:'center' }}>
                ...
                {canEdit && <button onClick={()=>openEdit(p)} style={{ background:'#F9F9F9', border:'1px solid #F1F1F4', borderRadius:8, padding:'5px 12px', cursor:'pointer', fontSize:12, fontWeight:600, color:'#252F4A', display:'flex', alignItems:'center', gap:5 }}><Edit2 size={11}/> Edit</button>}
              </div>
            </div>
            ...
            <div style={{ display:'flex', gap:8, alignItems:'center', flexWrap:'wrap' }}>
              <span style={{ fontSize:11, fontWeight:700, color:'#78829D' }}>RERA:</span>
            ...
            {reraWarn && (
              <div style={{ marginTop:8, display:'flex', alignItems:'center', gap:8, padding:'7px 12px', background:'#FFF8DD', border:'1px solid #F6C000', borderRadius:8 }}>
                <AlertTriangle size={13} style={{ color:'#F6C000' }}/>
                <span style={{ fontSize:12, fontWeight:600, color:'#9A6700' }}>RERA expires in {reradays} days</span>
              </div>
            )}
          </div>
        );
      ...
      <Modal open={open} onClose={()=>{ setOpen(false); setErrors({}); }}
        ...
        <div style={{ display:'flex', marginBottom:22 }}>
          {STEPS.map((s,i)=>(
            <div key={i} style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', gap:5, position:'relative' }}>
              ...
              <div style={{ position:'relative', zIndex:1, width:26, height:26, borderRadius:'50%', display:'flex', alignItems:'center', justifyContent:'center', fontSize:11, fontWeight:700, background:i<step?'#17C653':i===step?'#071437':'#F1F1F4', color:i<=step?'#fff':'#78829D' }}>
                {i<step?'✓':i+1}
              </div>
              <div style={{ fontSize:10.5, fontWeight:i===step?700:500, color:i===step?'#071437':'#99A1B7', textAlign:'center' }}>{s}</div>
            </div>
          ))}
        </div>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:13 }}>
            ...
            <F label="Joint Venture?">
              <div style={{ display:'flex', gap:10, height:36, alignItems:'center' }}>
                {[false,true].map(v=>(
                  <label key={String(v)} style={{ display:'flex', alignItems:'center', gap:7, cursor:'pointer', padding:'7px 16px', borderRadius:8, border:`2px solid ${form.is_jv===v?'#5014D0':'#F1F1F4'}`, background:form.is_jv===v?'#F1E8FF':'#fff' }}>
                    <input type="radio" checked={form.is_jv===v} onChange={()=>setForm(f=>({...f,is_jv:v}))} style={{ width:13,height:13 }}/>
                    <span style={{ fontSize:13, fontWeight:700, color:form.is_jv===v?'#5014D0':'#78829D' }}>{v?'Yes — Joint Venture':'No'}</span>
                  </label>
                ))}
              </div>
            </F>
          ...
          <div>
            ...
            <div style={{ background:'#FCFCFC', border:'1px solid #F1F1F4', borderRadius:12, padding:'14px 16px', marginBottom:16 }}>
              ...
              <div style={{ display:'flex', gap:12, marginBottom:12 }}>
                {[false,true].map(v=>(
                  <label key={String(v)} style={{ flex:1, display:'flex', alignItems:'center', gap:8, cursor:'pointer', padding:'8px 14px', borderRadius:8, border:`2px solid ${form.uses_wings===v?'#071437':'#F1F1F4'}`, background:form.uses_wings===v?'#F1F1F4':'#fff', justifyContent:'center' }}>
                    <input type="radio" checked={form.uses_wings===v} onChange={()=>onUsesWingsChange(v)} style={{ width:14,height:14 }}/>
                    <span style={{ fontSize:13, fontWeight:700, color:form.uses_wings===v?'#071437':'#78829D' }}>{v?'Has Wings (A, B, C…)':'No Wings — Single Block'}</span>
                  </label>
                ))}
              </div>
              {form.uses_wings && (
            ...
            <div style={{ background:'#FCFCFC', border:'1px solid #F1F1F4', borderRadius:12, padding:'14px 16px', marginBottom:16 }}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 2fr', gap:13, alignItems:'start' }}>
                ...
                <div style={{ paddingTop:28, fontSize:12.5, color:'#4B5675', lineHeight:1.7 }}>
                  ...
                  <span style={{ color:'#78829D', fontSize:12 }}>Example: G + 7 upper floors → enter 7</span>
                </div>
              </div>
            </div>
            ...
            {form.floor_configs.length > 0 && (
              <div style={{ background:'#fff', border:'1px solid #F1F1F4', borderRadius:12, overflow:'hidden', marginBottom:14 }}>
                <div style={{ padding:'10px 16px', background:'#F9F9F9', borderBottom:'1px solid #F1F1F4', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                ...
                <table style={{ width:'100%', borderCollapse:'collapse' }}>
                  ...
                  <tbody>
                    {form.floor_configs.map((fc,i)=>(
                      <tr key={fc.floor_no} style={{ borderBottom:'1px solid #F9F9F9', background:i%2===0?'#fff':'#FCFCFC' }}>
                        <td style={{ padding:'9px 14px' }}>
                          <span style={{ background:fc.floor_no===0?'#FFF8DD':'#F1F1F4', color:fc.floor_no===0?'#9A6700':'#071437', fontSize:12, fontWeight:700, padding:'3px 10px', borderRadius:8 }}>
                        ...
                        <td style={{ padding:'9px 14px', width:220 }}>
                          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                            <input style={{ ...inp, padding:'5px 9px', fontSize:12.5, width:90 }} type="number" min="0"
                              ...
                              placeholder="0"/>
                            <span style={{ fontSize:12, color:'#78829D' }}>
                              {Number(fc.unit_count)>0 ? `→ ${fc.unit_count} unit${Number(fc.unit_count)>1?'s':''}` : ''}
                            </span>
                          </div>
                        </td>
                        <td style={{ padding:'9px 14px', fontSize:11.5, color:'#99A1B7' }}>
                ...
                {errors.floor_configs && <div style={{ background:'#FFE2E5', padding:'8px 14px', fontSize:12, color:'#A10035' }}>{errors.floor_configs}</div>}
              </div>
            )}
            ...
            {totalUnits > 0 && (
              <div>
                {/* JV landowner note */}
                {form.is_jv && (
                  <div style={{ background:'#F1E8FF', border:'1px solid #C4B5FD', borderRadius:10, padding:'10px 14px', marginBottom:12, fontSize:12.5, color:'#5014D0', fontWeight:600 }}>
                    🏠 This is a Joint Venture project. Mark landowner units with the "LO" toggle — they will be locked from sales inventory.
                  </div>
                )}

                <div style={{ background:'#F1F1F4', border:'1px solid #DBDFE9', borderRadius:10, padding:'9px 14px', marginBottom:12, fontSize:12.5, color:'#071437' }}>
                  <strong>{totalUnits} total units</strong> · <span style={{ color:'#17C653' }}>{salableCount} salable</span>
                  {form.is_jv && landownerCount>0 && <> · <span style={{ color:'#5014D0' }}>{landownerCount} landowner (locked)</span></>}
                </div>
                          ...
                          {Object.entries(floorMap)
                            ...
                            .flatMap(([floorNo, units])=>
                              units.map((u,ui)=>{
                                ...
                                return (
                                  <tr key={u._key} style={{ borderBottom:'1px solid #F9F9F9', background:isLO?'#F5F0FF':ui%2===0?'#fff':'#FCFCFC' }}>
                                    {ui===0 && (
                                      <td rowSpan={units.length} style={{ padding:'8px 10px', fontSize:11.5, fontWeight:700, color:'#071437', background:'#F1F1F4', borderRight:'1px solid #F1F1F4', textAlign:'center', verticalAlign:'middle', whiteSpace:'nowrap' }}>
                                        {u.floor_label || (Number(floorNo)===0?'Ground':floorNo)}
                                      </td>
                                    )}
                                    <td style={{ padding:'5px 7px' }}>
                                      <input style={{ ...inp, padding:'4px 7px', fontSize:12, background:isLO?'#F1E8FF':'#fff' }}
                                        value={u.unit_no} onChange={e=>updateUnitRow(u._key,'unit_no',e.target.value)}/>
                                    </td>
                                    <td style={{ padding:'5px 7px', minWidth:90 }}>
                                      <select style={{ ...sel, padding:'4px 7px', fontSize:12, background:isLO?'#F1E8FF':'#fff', minWidth:88 }}
                                    ...
                                    <td style={{ padding:'5px 7px' }}>
                                      <input style={{ ...inp, padding:'4px 7px', fontSize:12, background:'#F9F9F9' }} type="number"
                                      ...
                                      <td style={{ padding:'5px 10px', textAlign:'center' }}>
                                        <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:4 }}>
                                          <button onClick={()=>toggleLandowner(u._key)}
                                            style={{ background:isLO?'#5014D0':'#F9F9F9', color:isLO?'#fff':'#78829D', border:`1px solid ${isLO?'#5014D0':'#F1F1F4'}`, borderRadius:6, padding:'3px 10px', cursor:'pointer', fontSize:11, fontWeight:700 }}>
                                            {isLO?'🔒 LO':'Mark LO'}
                                          </button>
                                          {isLO && (
                ...
                <div style={{ fontSize:11.5, color:'#78829D' }}>Built-up area = Carpet + Balcony (auto). You can override it.</div>
              </div>
            )}
          </div>
        )}
        ...
        {step===2 && (
          <div>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12 }}>
              ...
              <div style={{ display:'flex', gap:8, alignItems:'center' }}>
                <div style={{ fontSize:13, fontWeight:700, padding:'4px 12px', borderRadius:8, background:pctTotal===100?'#E8FFF3':'#FFE2E5', color:pctTotal===100?'#17C653':'#A10035', fontFamily:'monospace' }}>
                  {pctTotal}%
                </div>
                <button onClick={addMilestone} className="btn-secondary" style={{ fontSize:12 }}><Plus size={12}/> Add Stage</button>
              </div>
            </div>
            {errors.milestones && <div style={{ background:'#FFE2E5', border:'1px solid #FCA9BD', borderRadius:9, padding:'8px 12px', marginBottom:12, fontSize:12, color:'#A10035' }}>{errors.milestones}</div>}
            <table style={{ width:'100%', borderCollapse:'collapse' }}>
              <thead><tr style={{ background:'#F9F9F9', borderBottom:'2px solid #F1F1F4' }}>
              ...
              <tbody>
                {form.milestone_schedule.map((m,i)=>(
                  <tr key={m.id} style={{ borderBottom:'1px solid #F9F9F9' }}>
                    <td style={{ padding:'8px 12px', fontSize:12, color:'#78829D', width:32 }}>{i+1}</td>
                    ...
                    <td style={{ padding:'8px 12px' }}>
                      <button onClick={()=>removeMilestone(i)} style={{ background:'#FFE2E5', border:'1px solid #FCA9BD', borderRadius:6, padding:'3px 8px', cursor:'pointer', fontSize:11, fontWeight:600, color:'#A10035' }}>Remove</button>
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:13 }}>
            <F label="RERA Applicable" span={2}>
              <div style={{ display:'flex', gap:10 }}>
                {['Yes','No','Exempt'].map(v=>(
                  <label key={v} style={{ flex:1, display:'flex', alignItems:'center', gap:7, cursor:'pointer', padding:'8px 14px', borderRadius:8, border:`2px solid ${form.rera_applicable===v?'#071437':'#F1F1F4'}`, background:form.rera_applicable===v?'#F1F1F4':'#fff', justifyContent:'center' }}>
                    <input type="radio" value={v} checked={form.rera_applicable===v} onChange={set('rera_applicable')} style={{ width:14,height:14 }}/>
                    <span style={{ fontSize:13, fontWeight:700, color:form.rera_applicable===v?'#071437':'#78829D' }}>{v}</span>
                  </label>
                ))}
              </div>
            </F>
            ...
            {editingId && (
              <div style={{ gridColumn:'1/-1', borderTop:'1px solid #F9F9F9', paddingTop:14, marginTop:4 }}>
                ...
                {(form.approvals||[]).length===0
                  ? <div style={{ fontSize:12, color:'#99A1B7', textAlign:'center', padding:16 }}>No approvals recorded.</div>
                  : <table style={{ width:'100%', borderCollapse:'collapse' }}>
                    <thead><tr style={{ background:'#F9F9F9', borderBottom:'2px solid #F1F1F4' }}>
                    ...
                    <tbody>
                      {(form.approvals||[]).map((a,i)=>(
                        <tr key={a.id} style={{ borderBottom:'1px solid #F9F9F9', background:i%2===0?'#fff':'#FCFCFC' }}>
                          ...
                          <td style={{ padding:'7px 10px' }}>
                            <button onClick={()=>{ setApprovalForm({...a}); setEditApprovalId(a.id); setApprovalModal(true); }} style={{ background:'#F9F9F9', border:'1px solid #F1F1F4', borderRadius:6, padding:'2px 8px', cursor:'pointer', fontSize:11, fontWeight:600 }}>Edit</button>
                          </td>
                        </tr>
                      ))}
                    </tbody>
        ...
        {step===4 && (
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:13 }}>
            ...
            {pctTotal!==100 && (
              <div style={{ gridColumn:'1/-1', background:'#FFE2E5', border:'1px solid #FCA9BD', borderRadius:10, padding:'10px 14px', fontSize:12.5, color:'#A10035', fontWeight:600 }}>
                ⚠ Milestones total {pctTotal}% — must be 100%. Click Back to fix.
              </div>
            )}
            <div style={{ gridColumn:'1/-1', background:'#E8FFF3', border:'1px solid #A2E8BA', borderRadius:10, padding:'10px 14px', fontSize:12.5, color:'#17C653' }}>
